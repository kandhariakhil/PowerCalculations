...
clc
            ...
            for b = 0:floor(n/2) %Maximum number of bridged segments possible is half the number of segments
                ...
                while ((n-w*(m+b)>0) && (2*m+2*b-(n))<=0)
                    ...
                    if(COT<min_COT)
                        ...
                        COTopt(n) = (1/nu)*(((n*pi*R^3)/((n-wopt(n)*(mopt(n)+bopt(n)))*2*L*t^2))+(((mopt(n)+bopt(n))^4*(L/R)^3)/8))*(n/(wopt(n)*(mopt(n)/2)*((mopt(n)/2)+bopt(n))));
                        %COTopt(n) = (1/nu)*(((n*pi*R^3)/((n-wopt(n)*(mopt(n)+bopt(n)))*2*L*t^2))+(((mopt(n)+bopt(n))^4*(L/R)^3)/8))*(n/(wopt(n)*mopt(n)*(mopt(n)+bopt(n))));
                        %COTopt(n) = min_COT;
                    end
                    m=m+1;
                end
    ...
    %Calculate percentage of anchoring segments and ratio of moving to anchoring segments
    for i = min_segments:max_segments
        ratio(i) = (wopt(i)*(mopt(i)+bopt(i)))/(i-(wopt(i)*(mopt(i)+bopt(i))));
        %perc_anchoring(i) = ((i-wopt(i)*(mopt(i)+bopt(i)))/i)*100;
    end
    
    %calculate bending and compression power factors when optimized
    %{
    for i = min_segments:max_segments
        ...
        max_velocity(i) = wopt(i)*mopt(i)*(mopt(i)+bopt(i))/(i*L);
    end
    %}
